...
1. [重命名接口（区分 WAN / LAN）](#1-重命名接口区分-wan--lan)
2. [给 LAN 接口设置固定 IP](#2-给-lan-接口设置固定-ip)
3. [配置 DHCP Server（让 ROS 给电脑分 IP）](#3-配置-dhcp-server让-ros-给电脑分-ip)
4. [设置 WAN 接口自动获取上级光猫 IP](#4-设置-wan-接口自动获取上级光猫-ip)
5. [设置 NAT（让内网设备能上网）](#5-设置-nat让内网设备能上网)
6. [开启 DNS 转发（让 ROS 做内网 DNS）](#6-开启-dns-转发让-ros-做内网-dns)
7. [测试电脑](#7-测试电脑)
...
## 3. 配置 DHCP Server（让 ROS 给电脑分 IP）
